...
const eoepca = {
    name: 'eoepca',
    authorize_endpoint: 'https://test.185.52.193.87.nip.io/oxauth/restv1/authorize',
    isJWT: true,
    isValid: (params, creds) => {
        const { url_params } = params;
        return new Promise((resolve, reject) => {
            ...
            const options = [
                {
                    method: 'get',
                    url: `https://test.185.52.193.87.nip.io/oxauth/restv1/userinfo?access_token=${
                        creds.access_token
                    }`,
                },
                {
            ...
            axios
                ...
                .then(
                    axios.spread((resTokenInfo, resUserInfo) => {
                        ...
                        // check aud = client_id
                        const b = url_params.client_id === creds.id_token.aud;
                        // collect scope from tokeninfo
                        ...
                        creds.expiry = new Date(now.getTime() + creds.expires_in * 1000);
                        // set username
                        creds.username = creds.id_token.name;
